...
The VP9 extension provides `LibvpxVideoRenderer`, which uses palantir-libvpx (the SR-integrated VPx
...
```
cd $PALANTIR_CODE_ROOT/palantir/player/ExoPlayer
EXOPLAYER_ROOT="$(pwd)"
VP9_EXT_PATH="${EXOPLAYER_ROOT}/extensions/vp9/src/main"
```

* Set the location of Android NDK in an environment variable.

```
NDK_PATH="/android-ndk-r14b"
```
...
* Checkout the appropriate branches of libyuv (the scripts and
...
* Run a script that generates necessary configuration files for palantir-libvpx:
...
```
cd "${VP9_EXT_PATH}"/jni && \
${NDK_PATH}/ndk-build APP_ABI=arm64-v8a -j4
```
...
* Every time there is a change to the palantir-libvpx checkout:
