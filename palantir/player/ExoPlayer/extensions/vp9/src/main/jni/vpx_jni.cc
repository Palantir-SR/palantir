...
DECODER_FUNC(jlong, vpxInit, jboolean disableLoopFilter,
    ...
    if (palantir_cfg->decode_mode == DECODE_SR) {
        palantir_cfg->dnn_mode = ONLINE_DNN;
        palantir_cfg->dnn_runtime = GPU_FLOAT16;
    }
    if (palantir_cfg->decode_mode == DECODE_CACHE) {
        palantir_cfg->dnn_mode = ONLINE_DNN;
        palantir_cfg->dnn_runtime = GPU_FLOAT16;
        palantir_cfg->cache_mode = PROFILE_CACHE;
    }
    ...
    sprintf(dnn_name, "PALANTIR_S_B%d_F%d_S%d_deconv", numBlocks, numFilters, scale);
    ...
    switch (palantir_cfg->decode_mode) {
        case DECODE:
            if (palantir_cfg->save_latency) {
                sprintf(palantir_cfg->log_dir, "%s/log/%s", content_dir, input_video_name);
                _mkdir(palantir_cfg->log_dir);
            }
            break;
        case DECODE_SR:
            if (palantir_cfg->save_latency) {
                sprintf(palantir_cfg->log_dir, "%s/log/%s/%s", content_dir, input_video_name, dnn_name);
                _mkdir(palantir_cfg->log_dir);
            }
            break;
        case DECODE_CACHE:
            if (palantir_cfg->save_latency) {
                sprintf(palantir_cfg->log_dir, "%s/log/%s/%s/%s", content_dir, input_video_name,
                        dnn_name, profile_);
                _mkdir(palantir_cfg->log_dir);
            }
            break;
        case DECODE_BLOCK_CACHE:
            if (palantir_cfg->save_latency) {
                sprintf(palantir_cfg->log_dir, "%s/log/%s/%s/%s", content_dir, input_video_name,
                dnn_name, profile_);
                _mkdir(palantir_cfg->log_dir);
    ...
    if (vpx_load_palantir_cfg(context->decoder, palantir_cfg)) {
        LOGE("fail1");
    }
    ...
    sprintf(cache_profile_file, "%s/profile/%s/%s/%s.profile", content_dir, input_video_name, dnn_name, profile_);
    if (palantir_cfg->cache_mode == PROFILE_CACHE) {
        if (vpx_load_palantir_cache_profile(context->decoder, scale, cache_profile_file, palantir_cfg->num_patches_per_row, palantir_cfg->num_patches_per_column)) {
            LOGE("fail3");
            LOGE("palantir_cache profile: %s", cache_profile_file);
        }
    }

    remove_palantir_cfg(palantir_cfg);
