...
while getopts ":a:g:c:q:r:o:p:d:C:W:H:h" opt; do
    case $opt in
        h) _usage; exit 0;;
        a) algorithm="$OPTARG";;
        g) gop="$OPTARG";;
...
CUDA_VISIBLE_DEVICES=${gpu_index} python ${PALANTIR_CODE_ROOT}/palantir/player/setup_device.py --data_dir ${PALANTIR_DATA_ROOT} --content ${content} --video_name ${lr_video_name} --num_blocks ${num_blocks} --num_filters ${num_filters} --algorithm ${algorithm} --device_id ${device_id} --profile_name ${profile_name} --output_height ${output_height} ${py_checkpoint_ready_flag} --patch_width ${patch_width} --patch_height ${patch_height} --gop ${gop}