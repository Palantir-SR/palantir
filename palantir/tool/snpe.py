...
def snpe_convert_model(model, input_shape, checkpoint_dir, postfix=""):
    tf.compat.v1.disable_eager_execution()
    assert(not tf.executing_eagerly()) #note: output layer name is wrong in TF v1.3 with eager execution

    pb_name = '{}{}.pb'.format(model.name, postfix)
    opt_pb_name = '{}{}_opt.pb'.format(model.name, postfix)
    dlc_name = '{}{}.dlc'.format(model.name, postfix)
    qnt_dlc_name = '{}{}_quantized.dlc'.format(model.name, postfix)
    ...
    dlc_dict = {'model_name': model.name , \
            ...
            'qnt_dlc_path': os.path.join(checkpoint_dir, qnt_dlc_name), \
            'dlc_name': dlc_name, \
            'dlc_path': os.path.join(checkpoint_dir, dlc_name)}
    return dlc_dict
